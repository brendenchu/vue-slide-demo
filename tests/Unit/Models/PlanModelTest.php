...
it('has a subscriptions relationship', function () {
...
it('has an active subscriptions relationship', function () {
    $plan = Plan::factory()->create();
...
it('casts fields correctly', function () {
    ...
    expect($plan->price)->toBeString()->toBe('29.99')
        ...
        ->and($plan->created_at)->toBeInstanceOf(\Illuminate\Support\Carbon::class);
});
...
it('filters plans by interval with scope', function () {
...
it('formats price with custom currency symbol', function () {
    $plan = Plan::factory()->create(['price' => 29.99]);
